...
plotdir = "../Slides/figures" # figures directory (relative to R Code/)

# Suppress stray Rplots.pdf under Rscript
...
table[2,]   <- c(sqrt(min(cv.lasso.base$cvm)),sqrt(min(cv.lasso.poly$cvm)),sqrt(min(cv.lasso.int$cvm)))
colnames(table)<- c("Base","Polynomial","Interactions")
rownames(table)<- c("OLS","Lasso")
...
table[3,]   <- c(r2.CVave.lasso.base,r2.CVave.lasso.poly,r2.CVave.lasso.int)
table[4,]   <- c(r2.CVref.lasso.base,r2.CVref.lasso.poly,r2.CVref.lasso.int)
colnames(table)<- c("Base","Polynomial","Interactions")
rownames(table)<- c("OLS - average","OLS - refit",
