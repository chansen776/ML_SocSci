...
library(rpart)
library(xtable)
library(nnls)   # for short-stacking weights via NNLS + renormalize

set.seed(772023)

# (Data are loaded below via paths relative to this script's directory, so no
# hardcoded user path is needed. Run from the "R Code" dir, e.g. via run_all.R.)

#-----------------------------------------------------------------------------
# Helper: short-stacking weights via NNLS, renormalized to sum to 1.
# Matches the convention used by Stata pystacked/ddml and by R Code/401kATE.R.
...
# Columns of att/se.att: 1-9 per-learner (No Controls, Basic, Expansion,
#                        Lasso, Ridge, RF, Deep Tree, Shallow Tree, Tree CV)
#                        10 Best (min-MSE per arm)
#                        11 Stack (NNLS short-stack, renormalized)
att <- matrix(NA,4,11)
se.att <- matrix(NA,4,11)
RMSE.d <- matrix(NA,4,9)
RMSE.y <- matrix(NA,4,9)
trimmed <- matrix(NA,4,9)
stack_w_y_post  <- matrix(NA,4,9)   # short-stack weights for Y|X,D=0 per year
stack_w_d_post  <- matrix(NA,4,9)   # short-stack weights for D|X    per year
preds_post <- list()                # cross-fit predictions per year, saved at end
for(ii in 1:4){
  ...
  # Short-stack on the un-trimmed cross-fit predictions:
  #   - Y|X,D=0: NNLS of dy on per-learner predictions, D==0 rows only
  #   - D|X    : NNLS of treated on per-learner predictions, all rows
  w_y <- nnls_normalize(usedata$dy[usedata$treated == 0],
                        yGd0x.fit[usedata$treated == 0, , drop = FALSE])
  w_d <- nnls_normalize(usedata$treated, dGx.fit)
  yGd0_ss <- as.numeric(yGd0x.fit %*% w_y)
  dGx_ss  <- as.numeric(dGx.fit  %*% w_d)
  stack_w_y_post[ii, ] <- w_y
  stack_w_d_post[ii, ] <- w_d

  # Snapshot un-trimmed predictions for downstream diagnostics, then trim
  # propensity scores of 1 to .95 (per-learner AND short-stack).
  preds_post[[ii]] <- data.frame(
    model    = sprintf("m%02d", 3L + ii),                 # m04..m07
    obs_idx  = seq_len(n),
    treated  = usedata$treated,
    dy       = usedata$dy,
    pd       = pd.fit[, 1]
  )
  for(r in 1:9) {
    preds_post[[ii]][[paste0("yGd0_l", r)]] <- yGd0x.fit[, r]
    preds_post[[ii]][[paste0("dGx_l",  r)]] <- dGx.fit[, r]
  }
  preds_post[[ii]]$yGd0_ss <- yGd0_ss
  preds_post[[ii]]$dGx_ss  <- dGx_ss
  ...
  # Per-learner ATTs (cols 1-9), Best (col 10), Stack (col 11).
  best_d <- which.min(RMSE.d[ii, ])
  best_y <- which.min(RMSE.y[ii, ])
  att.num <- c(colMeans(((usedata$treated - dGx.fit)/((pd.fit%*%matrix(1,1,9))*(1-dGx.fit)))*
                        (usedata$dy-yGd0x.fit)) ,
               mean(((usedata$treated - dGx.fit[ , best_d])
                     /(pd.fit*(1-dGx.fit[ , best_d])))*
                          (usedata$dy-yGd0x.fit[ , best_y])),
               mean(((usedata$treated - dGx_ss)
                     /(pd.fit * (1 - dGx_ss))) *
                          (usedata$dy - yGd0_ss)))
  ...
  phihat <- cbind(((usedata$treated - dGx.fit)/((pd.fit%*%matrix(1,1,9))*(1-dGx.fit)))*
                         (usedata$dy-yGd0x.fit) ,
              ((usedata$treated - dGx.fit[ , best_d])
                    /(pd.fit*(1-dGx.fit[ , best_d])))*
                     (usedata$dy-yGd0x.fit[ , best_y]),
              ((usedata$treated - dGx_ss)
                    /(pd.fit * (1 - dGx_ss))) *
                     (usedata$dy - yGd0_ss))/att.den
...
table2 <- matrix(0, 22, 4)
table2[seq(1,22,2),]   <- t(att)
table2[seq(2,22,2),]   <- t(se.att)
...
# Short-stacking weights (rows = years 2004-2007; cols = 9 learners)
stack_w_post_disp <- rbind(stack_w_y_post, stack_w_d_post)
colnames(stack_w_post_disp) <- c("No Controls", "Basic", "Expansion",
                                 "Lasso (CV)", "Ridge (CV)",
                                 "Random Forest","Deep Tree",
                                 "Shallow Tree", "Tree (CV)")
rownames(stack_w_post_disp) <- c("wY 2004","wY 2005","wY 2006","wY 2007",
                                 "wD 2004","wD 2005","wD 2006","wD 2007")
cat("\nShort-stack weights (post-treatment models):\n")
print(round(stack_w_post_disp, 3))
...
# Ready to start DML estimation here
attP <- matrix(NA,1,11)
se.attP <- matrix(NA,1,11)
RMSE.dP <- matrix(NA,1,9)
RMSE.yP <- matrix(NA,1,9)
trimmedP <- matrix(NA,1,9)
stack_w_y_P  <- matrix(NA,1,9)
stack_w_d_P  <- matrix(NA,1,9)
preds_P <- NULL
for(ii in 1){
  ...
  for(r in 1:9) {
    preds_P[[paste0("yGd0_l", r)]] <- yGd0x.fit[, r]
    preds_P[[paste0("dGx_l",  r)]] <- dGx.fit[, r]
  }
  preds_P$yGd0_ss <- yGd0_ss
  preds_P$dGx_ss  <- dGx_ss
  ...
  best_d <- which.min(RMSE.dP[ii, ])
  best_y <- which.min(RMSE.yP[ii, ])
  att.num <- c(colMeans(((usedata$treated - dGx.fit)/((pd.fit%*%matrix(1,1,9))*(1-dGx.fit)))*
                          (usedata$dy-yGd0x.fit)) ,
               mean(((usedata$treated - dGx.fit[ , best_d])
                     /(pd.fit*(1-dGx.fit[ , best_d])))*
...
colnames(tableP)<- c("No Controls", "Basic", "Expansion",
                      "Lasso (CV)", "Ridge (CV)",
                      "Random Forest","Deep Tree",
                      "Shallow Tree", "Tree (CV)" , "Best", "Stack")
tableP = t(tableP)
...
cat("\nShort-stack weights (placebo model 2002):\n")
stack_w_P_disp <- rbind(stack_w_y_P, stack_w_d_P)
colnames(stack_w_P_disp) <- c("No Controls", "Basic", "Expansion",
                              "Lasso (CV)", "Ridge (CV)",
                              "Random Forest","Deep Tree",
                              "Shallow Tree", "Tree (CV)")
rownames(stack_w_P_disp) <- c("wY 2002","wD 2002")
print(round(stack_w_P_disp, 3))
...
# Stata Code/mwDiD.do; consumed by Python code/mwDiD_diagnostics.py.

preds_all <- do.call(rbind, c(list(preds_P), preds_post))
# Relative path matches the data load at the top of this file
# (load("../Data/mw_data_ch.RData")) -- assumes cwd is R Code/.
out_csv <- file.path("..", "Data", "mwDiD_R.csv")
write.csv(preds_all, out_csv, row.names = FALSE)
cat(sprintf("\nWrote %d rows of cross-fit predictions to %s\n",
            nrow(preds_all), normalizePath(out_csv, mustWork = FALSE)))
