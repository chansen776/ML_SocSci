...
# When run via Rscript, ggplot's default device writes Rplots.pdf.
# Suppress that noise when running non-interactively.
if (!interactive()) pdf(NULL)

######################### Options
dpl = FALSE                  # switch to save plots. Set to FALSE to just run code
plotdir = "../Slides/figures" # figures directory (used only if dpl)

######################### Read data
# Assumes working directory is the "R Code" subfolder of the repository.
...
for(k in 1:Kf) {
  indk = cvgroup == k
  K.train = train[!indk,]    # held-in fold from training sample
  K.test  = train[indk,]     # held-out fold from training sample
