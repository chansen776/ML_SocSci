...
* Replicate the treatment indicator under a model-specific name so that
* ddml's shortstack predicted variables (D_<varname>_ss_<rep>) do not
* collide when we save predictions from all five models in one dataset.
gen G04_m02 = G04
gen G04_m04 = G04
gen G04_m05 = G04
gen G04_m06 = G04
...
** cross-fitting (with short-stacking, matching §7 ATE pattern)
ddml crossfit , mname(m02) shortstack

*** Estimation results
ddml estimate, mname(m02) robust atet

*** Out-of-sample performance and stacking weights
ddml extract, mname(m02) show(mse)
ddml extract, mname(m02) show(ssweights)

*** Overlap plot for the short-stacked propensity score
ddml overlap, mname(m02) pslist(D_G04_m02_ss)
graph export ..\Slides\figures\Stata_mwStackedPS_m02.png , replace
graph close
...
ddml crossfit , mname(m04) shortstack

*** Estimation results
ddml estimate, mname(m04) robust atet

*** Out-of-sample performance and stacking weights
ddml extract, mname(m04) show(mse)
ddml extract, mname(m04) show(ssweights)

*** Overlap plot for the short-stacked propensity score
ddml overlap, mname(m04) pslist(D_G04_m04_ss)
graph export ..\Slides\figures\Stata_mwStackedPS_m04.png , replace
graph close
...
** cross-fitting (with short-stacking)
ddml crossfit , mname(m05) shortstack

*** Estimation results
ddml estimate, mname(m05) robust atet

*** Out-of-sample performance and stacking weights
ddml extract, mname(m05) show(mse)
ddml extract, mname(m05) show(ssweights)

*** Overlap plot for the short-stacked propensity score
ddml overlap, mname(m05) pslist(D_G04_m05_ss)
graph export ..\Slides\figures\Stata_mwStackedPS_m05.png , replace
graph close
...
** cross-fitting (with short-stacking)
ddml crossfit , mname(m06) shortstack

*** Estimation results
ddml estimate, mname(m06) robust atet

*** Out-of-sample performance and stacking weights
ddml extract, mname(m06) show(mse)
ddml extract, mname(m06) show(ssweights)

*** Overlap plot for the short-stacked propensity score
ddml overlap, mname(m06) pslist(D_G04_m06_ss)
graph export ..\Slides\figures\Stata_mwStackedPS_m06.png , replace
graph close
...
** cross-fitting (with short-stacking)
ddml crossfit , mname(m07) shortstack

*** Estimation results
ddml estimate, mname(m07) robust atet

*** Out-of-sample performance and stacking weights
ddml extract, mname(m07) show(mse)
ddml extract, mname(m07) show(ssweights)

*** Overlap plot for the short-stacked propensity score
ddml overlap, mname(m07) pslist(D_G04_m07_ss)
graph export ..\Slides\figures\Stata_mwStackedPS_m07.png , replace
graph close

*** Save data including cross-fit predictions and fold variables from all
*** five models for use in downstream diagnostics (Python code/mwDiD_diagnostics.py).
save "..\Data\mwDiD.dta" , replace
